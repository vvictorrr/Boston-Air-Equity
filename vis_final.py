...
POLLUTANT_OPTIONS = {"PM25": "pm25", "O3": "o3", "NO2": "no2"}
...
# VISUALIZATION 1 — High-level overview: ranked bar chart
...
# VISUALIZATION 2 — Drill-down scatter

def build_scatter(x_variable, pollutant):
    aq_annual = (
        monthly_df[monthly_df["parameter"] == pollutant]
        .groupby("county_name", as_index=False)
    ...
    x_label = next(
        (lbl for lbl, col in SCATTER_X_OPTIONS.items() if col == x_variable),
        x_variable,
    )
    y_label = POLLUTANT_LABELS.get(pollutant, pollutant)
    ...
    for _, row in plot_df.iterrows():
        fig.add_annotation(
            x=row[x_variable], y=row["mean_air_quality"],
...
def scatter_view(x, pollutant):
    return build_scatter(x, pollutant)


# VISUALIZATION 3 — Drill-down heatmap
def build_heatmap(pollutant):
    ...
    pivot = df.pivot_table(
        index="location_name", columns="month",
        values="monthly_avg_value", aggfunc="mean",
    )
    for m in range(1, 13):
        if m not in pivot.columns:
            pivot[m] = float("nan")
    pivot = pivot[sorted(pivot.columns)].sort_index(ascending=True)
...
def heatmap_view(pollutant):
    return build_heatmap(pollutant)


# VISUALIZATION 4 — Animated line chart via iframe
def build_animated_fig(pollutant):
    ...
    # Compute y-axis range with 15% padding so no point is ever clipped
    ymin  = df["mean_aq"].min()
    ymax  = df["mean_aq"].max()
    ypad  = (ymax - ymin) * 0.15
